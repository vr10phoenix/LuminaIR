...
root_data_path = '/kaggle/working/patches'
...
class MasterMultiCityDataset(Dataset):
    ...
    def __init__(self, root_dir):
        ...
        # 1. Step through every city folder (bangalore, mumbai, etc.)
        for city_name in sorted(os.listdir(root_dir)):
            city_path = os.path.join(root_dir, city_name)

            if os.path.isdir(city_path):
                # 2. Step through every sample patch folder inside the city
        ...
        print(f"[DATA ENGINE] Scanned root directory successfully.")
        print(f"[DATA ENGINE] Total cross-city sample folders locked: {len(self.sample_paths)}")
...
print("Cross-City PyTorch DataLoader Active and Locked.")